...
public sealed class VerifyBankTransferCommandHandler(IUnitOfWork uow, ICurrentUser current, IOrderLifecycleStore orderLifecycleStore) : IRequestHandler<VerifyBankTransferCommand, TResult>
{
    public async Task<TResult> Handle(VerifyBankTransferCommand r, CancellationToken ct)
    {
        ...
        var payment = await orderLifecycleStore.LockPaymentAsync(r.OrderId, ct);
        if (payment is not null && payment.Method != PaymentMethod.BankTransfer)
